    ...
    if reverse:
        ...
        for frameNumber in range(len(images)):
            if static and frameNumber > histLength:
                ...
                images[frameNumber] = fgbg.apply(images[frameNumber])
        if mustExistInPreviousFrames:
            for frameNumber in range(0,len(images)-1):
                for t in range(startingFrom):
                    if frameNumber + t >= len(images):
                        continue
                    images[frameNumber] = overlayFrames(images[frameNumber],images[frameNumber+t])
...
for runName in runNames:
    ...
    for eventNumber in eventsOfInterest: # iterates over all runNumber in runsOfInterest (note: runNumber is 0-indexed)
        ...
        thisEventImagesG = extractForegroundMask(False,True,True,thisEventImages,50,100,35,200)
        # cv2.imshow('test',thisEventImagesGGGG)
        # # cv2.imshow('test',cv2.merge([thisEventImagesGGGG,thisEventImagesGGGG,thisEventImagesGGGG]))
        # cv2.waitKey(0)
        # cv2.destroyAllWindows()
        # thisEventImagesGGG = extractForegroundMask(False,True,True,thisEventImages,10,100,9,5)
        # for frameNumber in range(len(thisEventImages)):
        ...
        #     thisEventImagesG[frameNumber] = np.concatenate([thisEventImagesG[frameNumber],thisEventImagesGGG[frameNumber],overlayFrames(thisEventImagesGGG[frameNumber],thisEventImagesG[-1])],1)
        ...
        # allCircles = np.zeros_like(np.concatenate([thisEventImages,thisEventImages,thisEventImages],2))
        ...
        for frameNumber in range(len(thisEventImages)):
            ...
            Images.append(np.concatenate([thisEventImages[frameNumber],thisEventImagesGg[frameNumber],allCircles[frameNumber]],1))
