...
for runName in runNames:
    ...
    for eventNumber in eventsOfInterest: # iterates over all runNumber in runsOfInterest (note: runNumber is 0-indexed)
        ...
        for frameNumber in range(len(thisEventImages)):
            thisEventImages[frameNumber] = cv2.cvtColor(cv2.normalize(thisEventImages[frameNumber],np.zeros_like(thisEventImages[frameNumber]),0,255,cv2.NORM_MINMAX),cv2.COLOR_BGR2GRAY)
...
writeAviVideo(videoName = folder+os.path.sep+'Very Test Video - '+folder,frameRate = 1,allImages = Images,openVideo = True,color = False)
