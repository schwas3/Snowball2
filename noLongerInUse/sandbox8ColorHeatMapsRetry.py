...
for runName in runNames:
    ...
    for eventNumber in eventsOfInterest: # iterates over all runNumber in runsOfInterest (note: runNumber is 0-indexed)
        ...
        Images3.append(np.zeros_like(thisEventImages[0,:,:,0]))
        Images4.append(np.zeros_like(thisEventImages[0,:,:,0]))
        ...
        frames = []
        thisEvent3 = extractForegroundMask(False,True,True,thisEventImages,50,100,0,200)
        thisEvent3 = [overlayFrames(thisEvent3s,thisEvent3[-1])for thisEvent3s in thisEvent3]
        blur = 15
        thisEvent3 = [cv2.GaussianBlur(thisEvent3s,(blur,blur),cv2.BORDER_DEFAULT)for thisEvent3s in thisEvent3]
        ballParkFrame = 0
        detectedFrame = 0
        for frameNumber in range(eventLength):
            if ballParkFrame == 0:
        ...
        thisEvent1 = thisEvent3
        thisEvent2 = thisEvent3
        ballParkFrame = 0
        for frameNumber in range(eventLength):
            if ballParkFrame == 0:
...
if openYorN:writeAviVideo(videoName = 'TestVideo',frameRate = 1,allImages = Images,openVideo = openYorN,color = True)
if False:
