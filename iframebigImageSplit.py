...
def makeBarGraph(labels,width,barLabels,barData,title,yLabel):
...
for groupName in groupNames:
    ...
    for runNumber in runsOfInterest: # iterates over all runNumber in runsOfInterest (note: runNumber is 0-indexed)
        ...
        mid = int(np.mean(np.sum(thisRunImages)))
        bottom = 2*mid-255
        if bottom > 0:
        ...
        for frameNumber in range(1,runLength+1): # iterates through every index in the range of the number of frames in the run
            ...
            blurredImages[frameNumber%runLength]=thisFrameImage
        ...
        for frameNumber in range(runLength,0,-1): # iterates in reverse
            bigImagesMemory[frameNumber%runLength] = bigImages[frameNumber%runLength]
            if frameNumber < runLength:
                bigImagesMemory[frameNumber] = np.multiply(bigImagesMemory[frameNumber],np.divide(bigImagesMemory[(frameNumber+1)%runLength],255))
        ...
        for frameNumber in range(runLength):
            compositeImages[frameNumber] = np.multiply(compositeImages[frameNumber],np.divide(bigImages[np.min([runLength,ballParkFrame+5])%runLength],255))
    ...
    makeBarGraph(labels,0.35,['Code Output','Answer Key'],[codeFrame,keyFrame],groupName[0:-8],'Detection Frame')
#     # txtName += groupName+','
