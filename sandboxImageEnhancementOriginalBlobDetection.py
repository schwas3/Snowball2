...
for runName in runNames:
    ...
    for eventNumber in eventsOfInterest: # iterates over all runNumber in runsOfInterest (note: runNumber is 0-indexed)
        ...
        for frameNumber in range(len(thisEventImages)):
            ...
            clahe = cv2.createCLAHE(clipLimit=2,tileGridSize=(40,40))
            ...
            thisEventImages[frameNumber] = np.concatenate([img,img_clahe],0)
        ...
        # continue
        # thisEventImages = np.subtract(255,thisEventImages)
        ...
        thisEventImages = [cv2.GaussianBlur(thisEventImagesGs,(blur,blur),cv2.BORDER_DEFAULT)for thisEventImagesGs in thisEventImages]
        # blur = 5
        ...
        for frameNumber in range(len(thisEventImages)):
            ...
            keypoints = detector.detect(thisEventImages[frameNumber])
