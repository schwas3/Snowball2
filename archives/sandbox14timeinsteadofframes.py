...
def makeBarHistGraphsSoloTime(labels,width,barData,title,makeBar: bool,makeHist:bool,folderH):
    if makeBar:
        ...
        ax.set_ylabel('Detection - Time Elapsed (seconds)',fontsize = 10)
        ax.set_xlabel('Event Number',fontsize = 10)
        ax.set_title(title,fontsize = 12)
        ax.set_xticks(x)
        ax.set_xticklabels(labels,fontsize = 6)
        ax.legend(fontsize = 8)
        ax.bar_label(rects,[str(round(i,2)) for i in barData], padding=2, fontsize = 6)
        fig.tight_layout()
        plt.ylim(0,30)
        # suffix = 1
    ...
    if makeHist:
        ...
        plt.title(title)
        plt.hist(barData,bins=30,range=(0,30),alpha=0.5,rwidth=0.9)
        plt.xlabel('Detection - Time Elapsed (seconds)'+' - Mean = '+str(round(np.mean(barData),2))+', Std = '+str(round(np.std(barData),2))+', N = '+str(len(barData)))
...
# print(runNames)
# runNames = [''] # the short name of the folder containing images (tif files)
notesContent = []
for runName in runNames:
    ...
    for eventNumber in eventsOfInterest: # iterates over all runNumber in runsOfInterest (note: runNumber is 0-indexed)
        ...
        codeTimeD.append(eval(thisEventFrameTimestamps[detectedFrame]+'-'+thisEventFrameTimestamps[0]))
    ...
    makeBarHistGraphsSolo(labels,0.35,codeFrame,folder+' - Frames - '+runName,True,True,'')
    makeBarHistGraphsSoloTime(labels,0.35,codeTimeD,folder+' - '+runName,True,True,'')
    # if True:
    #     txtName = runName
