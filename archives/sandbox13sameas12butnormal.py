...
writeAviVideo(videoName = 'Full Run - a - testvideo orig BackTrack VS Normal',frameRate = 1,allImages = Images,openVideo = True,color = False)
...
# writeAviVideo(videoName ='Frames 140 - 200 - a - control0 - 28',frameRate = 1,allImages = Images2,openVideo = False,color = False)
newImage = []
# for i in range(7):
#     for j in range(7):
#         if j == 0:
#             newImage1 = padEvent([Images[len(Images)-50+7*i]])[0]
#         else:
#             newImage1 = concatFrames(newImage1, padEvent([Images[len(Images)-50+7*i+j]])[0],1)
#     if i == 0:
#         newImage = newImage1
#     else:
#         newImage = concatFrames(newImage,newImage1,0)
# cv2.imwrite('testimage.jpg',newImage)
