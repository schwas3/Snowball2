...
def makeBarHistGraphsSolo(labels,width,barData,title,makeBar: bool,makeHist:bool,folderH):
    ...
    if makeHist:
        ...
        plt.title(title)
        plt.hist(barData,bins=20,range=(1,201),alpha=0.5,rwidth=0.9)
...
for runName in runNames:
    ...
    for eventNumber in eventsOfInterest: # iterates over all runNumber in runsOfInterest (note: runNumber is 0-indexed)
        ...
        theseImages = concatFrames(concatFrames(padEvent(frames),padEvent(thisEvent1),2),concatFrames(padEvent(thisEvent3),padEvent(thisEvent2),2),1)
        tStamp = []
        ...
        for frameNumber in range(eventLength):
            ...
            thisImages.append(cv2.resize(theseImages[frameNumber],(256,2*96)))
