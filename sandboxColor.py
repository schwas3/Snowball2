...
def padEvent(eventImages):
    ...
    # except:return np.pad(eventImages,[(0,0),(0,0),(1,1),(1,1)],'constant',constant_values = 255)
    return np.pad(eventImages,[(0,0),(1,1),(1,1),(1,1)],'constant',constant_values = 255)
...
def getEventsFromRun(runFolder): # str - name of grou p, str array - names of runs, str 2d array - timestamps in runs, str 2d array - filenames in runs
    ...
    for i in filename_RFG:
        ...
        runName_RFG, timestamp_RFG = tif_RFG.replace('.tiff','').replace('.bmp','').replace('.tif','').split('_')
...
for runName in runNames:
    ...
    for eventNumber in eventsOfInterest: # iterates over all runNumber in runsOfInterest (note: runNumber is 0-indexed)
        ...
        thisEvent1 = np.array(thisEvent1)
        thisEvent1 = np.array([cv2.merge([thisEvent11,thisEvent11,thisEvent11]) for thisEvent11 in thisEvent1])
        ...
        for i in range(len(theseImages)):
            thisImages.append(concatFrames(thisEventImages[i],cv2.merge([theseImages[i],theseImages[i],theseImages[i]]),1))
            thisImages[-1] = cv2.resize(thisImages[-1],(256,96))
# theseImages = concatFrames(frames,thisEvent1,2)
        ...
        for frameNumber in range(eventLength):
            # thisImages[frameNumber] = cv2.resize(thisImages[frameNumber],(256,96),interpolation=cv2.INTER_AREA)
...
writeAviVideo(videoName = folder+os.path.sep+'Full Runs - COLOR - '+folder,frameRate = 1,allImages = Images,openVideo = True,color = True)
