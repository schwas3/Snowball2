...
for groupName in groupNames:
    ...
    for runNumber in runsOfInterest: # iterates over all runNumber in runsOfInterest (note: runNumber is 0-indexed)
        ...
        ballParkImage = bigImages[ballParkFrame%runLength]
        images.append(cv2.resize(ballParkImage,(113,87)))
        frameVals = [np.sum(np.multiply(ballParkImage,compositeImages[ballParkFrame-30]))>0,np.sum(np.multiply(ballParkImage,compositeImages[ballParkFrame-29]))>0]
        for frameNumber in range(ballParkFrame-30,runLength):
            if detectedFrame == 0:
                frameVals.append((frameNumber+2)%(runLength+1) < 40 or np.sum(np.multiply(ballParkImage,compositeImages[(frameNumber+2)%runLength])))
                if frameVals[0] and frameVals[1] and frameVals[2]:
